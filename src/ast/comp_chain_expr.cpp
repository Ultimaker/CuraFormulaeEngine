...
[[nodiscard]] eval::Result ComparisonChainExpr::evaluate(
  ...
  for (size_t i = 0; i < operators.size(); i++) {
    ...
    case LessThenEqual:
      comparison_result =
          left_value_result.value() <= right_value_result.value();
      break;
    case GreaterThenEqual:
      comparison_result =
          left_value_result.value() >= right_value_result.value();
...
ComparisonChainExpr::freeVariables() const noexcept {
...
[[nodiscard]] bool
ComparisonChainExpr::deepEq(const Expr &other) const noexcept {
  if (const auto other_comp_chain_expr =
          dynamic_cast<const ComparisonChainExpr *>(&other)) {
    if (operators.size() != other_comp_chain_expr->operators.size() ||
        expressions.size() != other_comp_chain_expr->expressions.size()) {
      return false;
    }
    for (size_t i = 0; i < operators.size(); ++i) {
...
void ComparisonChainExpr::visitAll(
...
operator==(CuraFormulaeEngine::ast::ExprPtr lhs,
  ...
  return CuraFormulaeEngine::ast::make_expr_ptr<
      CuraFormulaeEngine::ast::ComparisonChainExpr>(
      std::move(expressions), std::vector{CuraFormulaeEngine::ast::Equals});
}
...
operator!=(CuraFormulaeEngine::ast::ExprPtr lhs,
  ...
  return CuraFormulaeEngine::ast::make_expr_ptr<
      CuraFormulaeEngine::ast::ComparisonChainExpr>(
      std::move(expressions), std::vector{CuraFormulaeEngine::ast::NotEquals});
}
...
operator<(CuraFormulaeEngine::ast::ExprPtr lhs,
  ...
  return CuraFormulaeEngine::ast::make_expr_ptr<
      CuraFormulaeEngine::ast::ComparisonChainExpr>(
      std::move(expressions), std::vector{CuraFormulaeEngine::ast::LessThan});
}
