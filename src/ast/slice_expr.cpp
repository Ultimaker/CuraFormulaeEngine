...
[[nodiscard]] std::string SliceExpr::toString() const noexcept {
  const auto start_index_str =
      start_index.has_value() ? start_index.value().toString() : "";
  const auto end_index_str =
      end_index.has_value() ? end_index.value().toString() : "";
  const auto step_size_str =
      step_size.has_value() ? step_size.value().toString() : "";
  return fmt::format("{}[{}:{}:{}]", array.toString(), start_index_str,
                     end_index_str, step_size_str);
}

[[nodiscard]] eval::Result
SliceExpr::evaluate(const env::Environment *environment) const noexcept {
  std::int64_t step_size_value = int64_t(1);
  if (step_size.has_value()) {
    const auto end_index_result =
        try_get<std::int64_t>(step_size.value().evaluate(environment));
  ...
  const auto array_result =
      try_get<std::vector<eval::Value>>(array.evaluate(environment));
  if (!array_result.has_value()) {
    return zeus::unexpected(array_result.error());
  }
  const auto &array_value = array_result.value();

  std::int64_t start_index_absolute_value =
      step_size_value > 0 ? 0 : int64_t(array_value.size()) - 1;
  if (start_index.has_value()) {
    const auto start_index_result =
        try_get<std::int64_t>(start_index.value().evaluate(environment));
    ...
    if (start_index_value < int64_t(0)) {
      start_index_absolute_value =
          int64_t(array_value.size()) - -start_index_value - int64_t(1);
      start_index_absolute_value =
          std::max(start_index_absolute_value, int64_t(0));
    } else {
      start_index_absolute_value = start_index_value;
      start_index_absolute_value =
          std::min(start_index_absolute_value,
                   static_cast<std::int64_t>(array_value.size()) - int64_t(1));
    }
    start_index_absolute_value =
        std::min(start_index_absolute_value,
                 static_cast<std::int64_t>(array_value.size()) - int64_t(1));
  ...
  std::int64_t end_index_absolute_value =
      step_size_value > int64_t(0)
          ? static_cast<std::int64_t>(array_value.size()) - int64_t(1)
          : int64_t(0);
  if (end_index.has_value()) {
    const auto end_index_result =
        try_get<std::int64_t>(end_index.value().evaluate(environment));
    if (!end_index_result.has_value()) {
      return zeus::unexpected(end_index_result.error());
    }
    ...
    if (end_index_value < int64_t(0)) {
      end_index_absolute_value =
          int64_t(array_value.size()) - -end_index_value - int64_t(1);
  ...
  if (step_size_value > 0) {
    for (std::int64_t i = start_index_absolute_value;
         i <= end_index_absolute_value; i += step_size_value) {
...
SliceExpr::freeVariables() const noexcept {
...
[[nodiscard]] bool SliceExpr::deepEq(const Expr &other) const noexcept {
  if (const auto other_slice_expr = dynamic_cast<const SliceExpr *>(&other)) {
    return array.deepEq(other_slice_expr->array) &&
           start_index.has_value() ==
               other_slice_expr->start_index.has_value() &&
           (!start_index.has_value() ||
            start_index.value().deepEq(
                other_slice_expr->start_index.value())) &&
           end_index.has_value() == other_slice_expr->end_index.has_value() &&
           (!end_index.has_value() ||
            end_index.value().deepEq(other_slice_expr->end_index.value())) &&
           step_size.has_value() == other_slice_expr->step_size.has_value() &&
           (!step_size.has_value() ||
            step_size.value().deepEq(other_slice_expr->step_size.value()));
  }
  return false;
}

void SliceExpr::visitAll(
