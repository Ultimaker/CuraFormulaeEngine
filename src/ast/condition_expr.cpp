...
[[nodiscard]] std::string ConditionExpr::toString() const noexcept {
  return fmt::format("({} if {} else {})", then_expr.toString(),
                     condition.toString(), else_expr.toString());
}

[[nodiscard]] eval::Result
ConditionExpr::evaluate(const env::Environment *environment) const noexcept {
...
ConditionExpr::freeVariables() const noexcept {
...
[[nodiscard]] bool ConditionExpr::deepEq(const Expr &other) const noexcept {
  if (const auto other_condition_expr =
          dynamic_cast<const ConditionExpr *>(&other)) {
    // clang-format off
