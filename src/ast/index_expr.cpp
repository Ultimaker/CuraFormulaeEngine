...
IndexExpr::evaluate(const env::Environment *environment) const noexcept {
...
IndexExpr::freeVariables() const noexcept {
...
[[nodiscard]] bool IndexExpr::deepEq(const Expr &other) const noexcept {
  if (const auto other_index_expr = dynamic_cast<const IndexExpr *>(&other)) {
    return array.deepEq(other_index_expr->array) &&
           index.deepEq(other_index_expr->index);
  }
  return false;
}

void IndexExpr::visitAll(
