...
[[nodiscard]] std::string ListComprehensionExpr::toString() const noexcept {
  const auto loops_str =
      loops | ranges::views::transform([](const auto &loop) {
        if (loop.conditions.empty()) {
          return fmt::format("for {} in {}", loop.iterator_key.toString(),
                             loop.iterable.toString());
        }
        const auto conditions_str =
            loop.conditions |
            ranges::views::transform([](const auto &condition) {
              return fmt::format("if {}", condition.toString());
            }) |
            ranges::views::join(ranges::views::c_str(" ")) |
            ranges::to<std::string>();
        return fmt::format("for {} in {} {}", loop.iterator_key.toString(),
                           loop.iterable.toString(), conditions_str);
      }) |
      ranges::views::join(ranges::views::c_str(" ")) |
      ranges::to<std::string>();
...
ListComprehensionExpr::handle_loop(const size_t loop_index,
                                   env::LocalEnvironment &local_environment,
  ...
  const auto iterable_result = try_get<std::vector<eval::Value>>(
      loop.iterable.evaluate(&local_environment));
  ...
  for (const auto &element : iterable_value) {
    ...
    if (loop_index + 1 == loops.size()) {
      ...
      const auto loop_err =
          handle_loop(loop_index + 1, local_environment, results);
...
[[nodiscard]] eval::Result ListComprehensionExpr::evaluate(
    const env::Environment *environment) const noexcept {
...
ListComprehensionExpr::freeVariables() const noexcept {
...
[[nodiscard]] bool
ListComprehensionExpr::deepEq(const Expr &other) const noexcept {
  if (const auto other_list_comprehension =
          dynamic_cast<const ListComprehensionExpr *>(&other)) {
    if (loops.size() != other_list_comprehension->loops.size()) {
      return false;
    }

    for (const auto [loop, other_loop] :
         ranges::views::zip(loops, other_list_comprehension->loops)) {
      ...
      for (const auto [condition, other_condition] :
           ranges::views::zip(loop.conditions, other_loop.conditions)) {
...
void ListComprehensionExpr::visitAll(
