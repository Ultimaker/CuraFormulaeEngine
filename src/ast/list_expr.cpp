...
ListExpr::evaluate(const env::Environment *environment) const noexcept {
...
ListExpr::freeVariables() const noexcept {
...
[[nodiscard]] bool ListExpr::deepEq(const Expr &other) const noexcept {
  if (auto other_list_expr = dynamic_cast<const ListExpr *>(&other)) {
    ...
    for (const auto [element, other_element] :
         ranges::views::zip(elements, other_list_expr->elements)) {
