...
[[nodiscard]] std::string DivExpr::getOpIdentifier() const noexcept
{
    return "/";
}

eval::Result DivExpr::evaluate(eval::Value& lhs, eval::Value& rhs) const noexcept
{
    return lhs / rhs;
}
...
{
    return CuraFormulaeEngine::ast::make_expr_ptr<CuraFormulaeEngine::ast::DivExpr>(std::move(lhs), std::move(rhs));
}
