...
[[nodiscard]] std::string BinaryExpr::toString() const noexcept {
  return fmt::format("({} {} {})", lhs.toString(), getOpIdentifier(),
                     rhs.toString());
}

[[nodiscard]] eval::Result
BinaryExpr::evaluate(const env::Environment *environment) const noexcept {
...
BinaryExpr::freeVariables() const noexcept {
  auto lhs_free_variables = lhs.freeVariables();
  auto rhs_free_variables = rhs.freeVariables();
  lhs_free_variables.insert(rhs_free_variables.begin(),
                            rhs_free_variables.end());
