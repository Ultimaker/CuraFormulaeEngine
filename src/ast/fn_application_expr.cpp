...
[[nodiscard]] std::string FnApplicationExpr::toString() const noexcept {
  auto args_str =
      args |
      ranges::views::transform([](const auto &arg) { return arg.toString(); }) |
...
[[nodiscard]] eval::Result FnApplicationExpr::evaluate(
...
FnApplicationExpr::freeVariables() const noexcept {
...
[[nodiscard]] bool FnApplicationExpr::deepEq(const Expr &other) const noexcept {
  if (const auto &other_fn_application =
          dynamic_cast<const FnApplicationExpr *>(&other)) {
