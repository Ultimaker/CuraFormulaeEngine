...
eval::Result
ExprPtr::evaluate(const env::Environment *environment) const noexcept {
...
std::string ExprPtr::toString() const noexcept { return ptr->toString(); }
...
CuraFormulaeEngine::ast::ExprPtr::operator[](ExprPtr other) {
  return CuraFormulaeEngine::ast::make_expr_ptr<IndexExpr>(std::move(*this),
                                                           std::move(other));
}
