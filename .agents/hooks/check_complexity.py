...
def git_base_ref() -> str:
    for candidate in ("origin/main", "origin/master", "main", "master"):
        res = subprocess.run(["git", "merge-base", "HEAD", candidate],
                             capture_output=True, text=True)
...
def check(staged: bool, advisory: bool, only=None):
    ...
    for path in paths:
        ...
        before_base = (complexity_of(git_show(base_ref, path), suffix)
                       if base_ref != "HEAD" else {})
        ...
        for name, ccn in sorted(after.items()):
            was = before.get(name)
            was_base = before_base.get(name)
            if was_base is None:
                short_name = name.split("::")[-1]
                for k, v in before_base.items():
