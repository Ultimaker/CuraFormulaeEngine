...
Description: Finds the cross-repository surfaces of `CuraFormulaeEngine` that no file-extension detector can see, and files them as `ecosystem_contract` observations.
...
You are investigating what `CuraFormulaeEngine` promises to, and expects from, the
