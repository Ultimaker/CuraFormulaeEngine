...
struct ExprPtr final : Expr {
  ...
  ExprPtr(std::unique_ptr<Expr> ptr) : ptr(std::move(ptr)) {}
  ...
  template <typename... Ts> ExprPtr operator()(Ts... args);
};

template <typename T, typename... Ts> ExprPtr make_expr_ptr(Ts... args) {
