...
template <typename T> struct PrimaryExpr : public Expr {
  T value;

  explicit PrimaryExpr(T value) : value(value) {}

  [[nodiscard]] eval::Result
  evaluate(const env::Environment *) const noexcept final {
    return value;
  }
  ...
  void
  visitAll(std::function<void(const Expr &)> visitor) const noexcept final {
    visitor(*this);
  }
};
