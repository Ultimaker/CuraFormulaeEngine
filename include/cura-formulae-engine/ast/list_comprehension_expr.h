...
struct ListComprehensionExpr final : Expr {
  struct loop {
    ...
    loop(ExprPtr iterator_key, ExprPtr iterable,
         std::vector<ExprPtr> conditions)
        : iterator_key(std::move(iterator_key)), iterable(std::move(iterable)),
  ...
  ListComprehensionExpr(ExprPtr iterator, std::vector<loop> loops)
      : iterator(std::move(iterator)), loops(std::move(loops)) {}
