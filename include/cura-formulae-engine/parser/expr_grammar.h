...
struct ExprGrammar : lexy::expression_production {
  ...
  static constexpr auto op_condition = lexy::dsl::op(
      LEXY_LIT("if") >> lexy::dsl::p<NestedGrammar> + LEXY_LIT("else"));
  ...
  static constexpr auto value = lexy::callback<ast::ExprPtr>(
      ...
      [](ast::ExprPtr &&lhs, lexy::op<op_condition>, ast::ExprPtr &&condition,
         ast::ExprPtr &&rhs) {
        return ast::ExprPtr(std::make_unique<ast::ConditionExpr>(
            std::move(lhs), std::move(condition), std::move(rhs)));
      });
};
