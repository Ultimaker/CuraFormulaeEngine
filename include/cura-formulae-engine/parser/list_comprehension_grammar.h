...
struct ListComprehensionGrammar : lexy::token_production {
  ...
  struct condition_chain : token_production {
    static constexpr auto rule = lexy::dsl::list(
        LEXY_LIT("if") >> lexy::dsl::whitespace(lexy::dsl::ascii::space) +
                              lexy::dsl::p<MathExprGrammar>);
    static constexpr auto value = lexy::fold_inplace<std::vector<ast::ExprPtr>>(
        []() { return std::vector<ast::ExprPtr>{}; },
        [](auto &&conditions, ast::ExprPtr &&condition) {
  ...
  struct loop : token_production {
    static constexpr auto rule = lexy::dsl::list(
        LEXY_LIT("for") >> lexy::dsl::whitespace(lexy::dsl::ascii::space) +
                               lexy::dsl::p<VariableGrammar> +
                               lexy::dsl::whitespace(lexy::dsl::ascii::space) +
                               LEXY_LIT("in") +
                               lexy::dsl::whitespace(lexy::dsl::ascii::space) +
                               lexy::dsl::p<MathExprGrammar> +
                               lexy::dsl::whitespace(lexy::dsl::ascii::space) +
                               lexy::dsl::opt(lexy::dsl::p<condition_chain>));

    static constexpr auto value =
        lexy::fold_inplace<std::vector<ast::ListComprehensionExpr::loop>>(
            []() { return std::vector<ast::ListComprehensionExpr::loop>{}; },
            [](auto &&loops, ast::ExprPtr &&iterator_key,
               ast::ExprPtr &&iterable, lexy::nullopt) {
              loops.emplace_back(ast::ListComprehensionExpr::loop(
                  std::move(iterator_key), std::move(iterable),
                  std::vector<ast::ExprPtr>{}));
            },
            [](auto &&loops, ast::ExprPtr &&iterator_key,
               ast::ExprPtr &&iterable,
  ...
  static constexpr auto rule =
      lexy::dsl::p<MathExprGrammar> + lexy::dsl::opt(lexy::dsl::p<loop>);
  static constexpr auto value = lexy::callback<ast::ExprPtr>(
      [](auto &&expr, lexy::nullopt) { return std::move(expr); },
      [](auto &&expr, auto &&loops) {
        return ast::ExprPtr(std::make_unique<ast::ListComprehensionExpr>(
            std::move(expr), std::move(loops)));
      });
};
