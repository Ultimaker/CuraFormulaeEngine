...
struct BoolGrammar : lexy::token_production {
  struct TrueGrammar {
    ...
    static constexpr auto value = lexy::callback<ast::ExprPtr>(
        []() { return ast::ExprPtr(std::make_unique<ast::BoolExpr>(true)); });
  };

  struct FalseGrammar {
    static constexpr auto rule = LEXY_LIT("False");
    static constexpr auto value = lexy::callback<ast::ExprPtr>(
        []() { return ast::ExprPtr(std::make_unique<ast::BoolExpr>(false)); });
  };

  static constexpr auto rule =
      lexy::dsl::p<TrueGrammar> | lexy::dsl::p<FalseGrammar>;
