...
struct ApplyFnApplicationExpr final : ApplyExpr {
  ...
  ast::ExprPtr apply(ast::ExprPtr &&variable) override {
    return {std::make_unique<ast::FnApplicationExpr>(std::move(variable),
                                                     std::move(args))};
  }
};
...
struct VariableOrFnApplicationGrammarOrArrayIndexingGrammar
  ...
  struct FnApplicationGrammar : token_production {
    struct FnApplicationGrammarInner : token_production {
      static constexpr auto whitespace =
          lexy::dsl::whitespace(lexy::dsl::ascii::space);
      static constexpr auto rule = lexy::dsl::round_bracketed.opt_list(
          lexy::dsl::p<NestedGrammar>,
          lexy::dsl::ignore_trailing_sep(lexy::dsl::comma));
      static constexpr auto value =
          lexy::as_list<std::vector<ast::ExprPtr>> >>
          lexy::callback<std::unique_ptr<ApplyExpr>>(
              [](lexy::nullopt = {}) -> std::unique_ptr<ApplyExpr> {
                return std::make_unique<ApplyFnApplicationExpr>();
              },
              [](std::vector<ast::ExprPtr> args) -> std::unique_ptr<ApplyExpr> {
                return std::make_unique<ApplyFnApplicationExpr>(
                    std::move(args));
              });
    };
    ...
    static constexpr auto rule = lexy::dsl::peek(lexy::dsl::lit_c<'('>) >>
                                 lexy::dsl::p<FnApplicationGrammarInner>;
  ...
  struct IndexOrSliceGrammar : token_production {
    static constexpr auto rule =
        ...
        lexy::dsl::square_bracketed(
            lexy::dsl::p<SliceElementColon> +
            lexy::dsl::times<2>(lexy::dsl::if_(
                lexy::dsl::colon >> lexy::dsl::p<SliceElementColonOrBracket>)));
    static constexpr auto value = lexy::callback<std::unique_ptr<ApplyExpr>>(
        [](std::optional<ast::ExprPtr> &&index) {
          return std::make_unique<IndexExpr>(std::move(index));
        },
        [](std::optional<ast::ExprPtr> &&start_index,
           std::optional<ast::ExprPtr> &&end_index) {
          return std::make_unique<ApplySliceExpr>(
              std::move(start_index), std::move(end_index), std::nullopt);
        },
        [](std::optional<ast::ExprPtr> &&start_index,
           std::optional<ast::ExprPtr> &&end_index,
           std::optional<ast::ExprPtr> &&step) {
          return std::make_unique<ApplySliceExpr>(
              std::move(start_index), std::move(end_index), std::move(step));
        });
  };

  struct List : token_production {
    static constexpr auto rule = lexy::dsl::list(
        lexy::dsl::p<FnApplicationGrammar> | lexy::dsl::p<IndexOrSliceGrammar>);
    static constexpr auto value =
        lexy::as_list<std::vector<std::unique_ptr<ApplyExpr>>>;
  };

  static constexpr auto
      rule = lexy::dsl::p<VariableGrammar> >>
             lexy::dsl::if_(lexy::dsl::peek(lexy::dsl::lit_c<'['> |
                                            lexy::dsl::lit_c<'('>) >>
                            lexy::dsl::p<List>);
  static constexpr auto value = lexy::callback<ast::ExprPtr>(
      [](auto &&expr) { return std::forward<decltype(expr)>(expr); },
      [](auto &&expr, lexy::nullopt) {
        return std::forward<decltype(expr)>(expr);
      },
      [](auto &&expr,
         const std::vector<std::unique_ptr<ApplyExpr>> &apply_exprs) {
